...
//			cout<<nw<<" ";
	...
	void solve()
	{
		...
		int testid=read(),op=0,q=5e5,n=5e5,C=5e5;
		if(is(1,10))n=testid*10,C=testid;
		if(is(11,20))n=5000,C=rd(5,30);
		if(is(21,30))n=5e5,C=5000;
		if(is(31,40))n=5e5,C=n;
		cout<<n<<" "<<q<<"\n";
		for(int i=1;i<=n;i++)cout<<rd(0,1)<<" ";
		puts("");
		for(int i=1;i<=n;i++)cout<<rd(1,C)<<" ";
		puts("");
		while(q--)
		{
